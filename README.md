...
```bash
./scripts/generate_known_versions.py 0.0.50
```
...
version = "0.0.50"
known_versions = [
  "0.0.50|linux_arm64|<sha256>|<size>",
  "0.0.50|linux_x86_64|<sha256>|<size>",
  "0.0.50|macos_arm64|<sha256>|<size>",
  "0.0.50|macos_x86_64|<sha256>|<size>",
]
```
...
```bash
./scripts/generate_known_versions.py 0.0.50 --platform macos_arm64 --platform linux_x86_64
./scripts/generate_known_versions.py 0.0.50 --entries-only
```
...
- create an annotated tag like `v0.1.10`
- push `main`
- push the tag
...
```bash
mise run release-dry-run 0.1.10
mise run release -- --skip-checks 0.1.10
```
